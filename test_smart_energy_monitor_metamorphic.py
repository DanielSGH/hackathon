...
def test_metamorphic_summary(energy_readings: list[EnergyReading.EnergyReading] = None):
    if energy_readings == None:
        ...
        for i in range(len(energy_readings)):
            sem.process_reading(energy_readings[i])
...
def test_metamorphic_order(energy_readings: list[EnergyReading.EnergyReading] = None):
    if energy_readings == None:
        ...
        sem0 = SmartEnergyMonitor(tariffservice, alertservice, anomalythreshold)
        for i in range(len(energy_readings)):
            sem0.process_reading(energy_readings[i])
        sem1 = SmartEnergyMonitor(tariffservice, alertservice, anomalythreshold)
        for i in range(len(energy_readings)):
            sem1.process_reading(energy_readings[i])
